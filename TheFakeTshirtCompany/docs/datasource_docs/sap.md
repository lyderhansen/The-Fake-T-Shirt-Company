...
```spl
index=fake_tshrt sourcetype="FAKE:sap:auditlog"
...
### 2. Failed operations
```spl
index=fake_tshrt sourcetype="FAKE:sap:auditlog" status="E"
...
```spl
index=fake_tshrt sourcetype="FAKE:sap:auditlog" demo_id=dead_letter_pricing
| table _time, user, tcode, description, doc_number, details
```

### 4. Nightly batch job monitoring
```spl
index=fake_tshrt sourcetype="FAKE:sap:auditlog" dialog_type="BTC"
| table _time, user, tcode, description, details
```

### 5. User login audit
```spl
index=fake_tshrt sourcetype="FAKE:sap:auditlog" (description="*Login*" OR description="*Logon*")
